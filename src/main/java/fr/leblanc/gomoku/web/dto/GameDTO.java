...
public record GameDTO(Long id, Integer boardSize, Set<MoveDTO> moves, Boolean strikeEnabled, Boolean displayAnalysis, Boolean displayEvaluation,
	...
	public GameDTO(Game game, UserSettings settings) {
		this(game.getId(), game.getBoardSize(), game.getMoves().stream().map(MoveDTO::new).collect(Collectors.toSet()),
				settings.isStrikeEnabled(), settings.isDisplayAnalysis(), settings.isDisplayEvaluation(), settings.getMinMaxDepth(),
